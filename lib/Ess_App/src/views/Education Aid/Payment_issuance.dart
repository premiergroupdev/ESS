...
class Payment_issuranceAidDataScreen extends StatefulWidget {
  const Payment_issuranceAidDataScreen({super.key});

  @override
  State<Payment_issuranceAidDataScreen> createState() => _AidDataScreenState();
}

class _AidDataScreenState extends State<Payment_issuranceAidDataScreen> {
  ...
  Future<void> fetchAidData() async {
    final url = Uri.parse(
        'https://premierspulse.com/ess/scripts/aid_payment_issue.php?EMPCODE=${currentUser?.userId.toString()}');
    final headers = {
  ...
  Future<void> close(String id, int index) async {

    final url = Uri.parse(
        'https://premierspulse.com/ess/scripts/update_education_voucher_status.php?voucher_id=${id}&status=closed');
    final headers = {
      'Authorization': 'Basic Basic RVNTOngyRnN0VnN5eg==',
      'Accept': 'application/json',
    };
                      ...
                      child: Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Row(
                              children: [
                                Text("Vouncher Amount : ", style: TextStyle(fontSize: 11, color: Colors.black),),
                                Text("${item['voucher_amount']}", style: TextStyle(fontSize: 13, color: Colors.black, fontWeight: FontWeight.bold),),
                              ],
                            ),
                            SizedBox(height: 5,),
                            Row(
                              children: [
                                InkWell(
                                    onTap:() async {
                                      if (item['voucher_image'] != null && item['voucher_image'].toString().isNotEmpty) {
                                        final Uri uri = Uri.parse(item['voucher_image'] );
                                        await launchUrl(
                                        uri,
                                    ...
                                    child: Text("See Vouncher Attachments", style: TextStyle(fontSize: 11, color: Colors.black, fontWeight: FontWeight.bold,     decoration: TextDecoration.underline, // 👈 underline added
                                    ))),
                              //  Text("${item['voucher_image']}", style: TextStyle(fontSize: 13, color: Colors.black, fontWeight: FontWeight.bold),),
                            ...
                            Wrap(
                              ...
                              children: [
                                ...
                                _buildStatusChip(
                                    "Dept Head", item["voucher_dept_head_approval"]),
