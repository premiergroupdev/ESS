...
class FinalAidDataScreen extends StatefulWidget {
  const FinalAidDataScreen({super.key});

  @override
  State<FinalAidDataScreen> createState() => _AidDataScreenState();
}

class _AidDataScreenState extends State<FinalAidDataScreen> {
  ...
  Future<void> fetchAidData() async {
    final url = Uri.parse(
        'https://premierspulse.com/ess/scripts/fetch_my_aid.php?EMPCODE=${currentUser?.userId.toString()}&api_type=ceo');
    final headers = {
                                  ...
                                  onTap: () async {
                                    ...
                                    bool? shouldRemove = await   showDialog(
                                      ...
                                      builder: (BuildContext context) {
                                        return  CeoPopupDialog( title: 'hr_level2', fnf_id: item['aid_id'], datalist: aidDataList, name: item['employee_name'],
                                    ...
                                    if (shouldRemove == true) {
                                      setState(() {
                                        if(aidDataList.isNotEmpty) {
                                          aidDataList.remove(item);
                                        }// 0 ho ya koi aur, safe rahega
                                      });
                                    }
