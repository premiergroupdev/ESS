...
class ItApprovalPopup extends StatefulWidget {
  final List<dynamic> datalist;
  final List<dynamic> hodlist;
  final String name;
  final String fnf_id;

  ItApprovalPopup({Key? key, required this.hodlist, required this.name , required this.fnf_id, required this.datalist}) : super(key: key);

  @override
  State<ItApprovalPopup> createState() => _ProfessionalPopupDialogState();
}

class _ProfessionalPopupDialogState extends State<ItApprovalPopup> {
  ...
  Future<void> Apicall2() async {
    ...
    final url = Uri.parse(
      'https://premierspulse.com/ess/scripts/update_fnf_linemanager.php'
          ...
          '&username=${currentUser?.userName ?? ""}' // ✅ use correct field name and default empty
          '&status=${_selectedStatus == "Accept" ? "approved" : _selectedStatus == "Reject" ? "rejected" : "pending"}' // ✅ nested ternary fixed
          '&approval_type=it_approval'
